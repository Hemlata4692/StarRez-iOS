...
- (void)loginUser:(LoginModel *)userLogin onSuccess:(void (^)(id))success onFailure:(void (^)(id))failure {
    ...
    NSString *parameters = [NSString stringWithFormat:@"SELECT en.[entryid], en.[PinNumber], ed.[Email], en.[NameLast], en.[NameFirst], en.[NameTitle], bk.[RoomLocationID], bk.[RoomSpaceID] FROM [Entry] AS en LEFT JOIN [EntryAddress] AS ed ON ed.[entryid] = en.[entryid] LEFT JOIN [Booking] as bk ON bk.[entryid] = en.[entryid] WHERE ed.[Email] ='%@' AND en.[PinNumber] = '%@' AND bk.EntryStatusEnum = '5' AND ed.[AddressTypeID] = 0",userLogin.userEmailId,userLogin.password];
